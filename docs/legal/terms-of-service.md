...
1.3 **Your installation, your responsibility.** The Software is self-hosted: you install and operate it on hardware you control. You may permit other persons — who may include members of your household — to access your Instance; they do so under your supervision and on your responsibility (including through any restricted-mode features you configure under clause 3.4), and you are responsible for all activity conducted through your Instance.
...
1.5 **Presentation at install.** These Terms are presented at installation together with a separate, prominent choice about community sharing that you must make for yourself: no option is pre-selected, and no sharing occurs before you make it (see clause 7.4). Accepting these Terms and making that sharing choice are distinct steps.
...
- "**Merit**" means the non-monetary creator-reputation record described in clause 9.
...
- "**Operator Services**" means the limited online services we operate in connection with the Software, being: distribution of the Software; the Community Library catalogue and index (served through our distribution infrastructure, clause 6.4); the publish service, publisher accounts and the Merit record (including sign-in for publishing); and the improvement-sharing endpoint that receives de-identified improvement patterns shared under clause 7.4. Operator Services do not include hosting or operating your Instance. Operator Services also do not include community discussion venues: community discussion takes place on Discord, a third-party platform operating under its own terms, and the Vaenyx Discord community is not an Operator Service.
...
3.2 **We do not host your Instance.** We supply software and the limited Operator Services. We do not operate or monitor your Instance and have no remote access to it; we receive data from your Instance only through the channels described in clause 7.2 — one of which (de-identified community sharing, clause 7.4) operates only if you affirmatively turn it on, at the install-time sharing choice or later in Settings. Your Instance's availability, performance and security depend on your hardware, your configuration and the Third-Party Services you connect.

3.3 **Local-first, honestly stated.** Your Data is stored on your device; we do not reach into your Instance to collect it, and data reaches us only through the channels described in clause 7.2 (including, if you have turned it on through the sharing choice in clause 7.4, the community-sharing channel). The Software's model backend is pluggable: if you connect a cloud AI model provider, the content you send in chats (and associated context) **will leave your device** and be transmitted to that provider under that provider's terms and at your cost (see clause 6). If you use only a local model, model processing occurs on your own device. These Terms never promise that no data leaves your device; what leaves depends on what you connect and what you choose to share or publish.
...
5.3 **Remote access.** The Software can integrate with a third-party remote-access service (currently Tailscale Funnel) using **your own** account with that provider. To provide this integration, the Software may install a bundled third-party remote-access client on your machine (including a Windows service), or may invoke a client you have installed yourself; any such client remains a third-party component governed by its provider's own licence and terms, and any bundled client is disclosed in the THIRD_PARTY_NOTICES document (clause 4.4). If you enable remote access:
...
6.1 **Your choice, your contract, your cost.** The Software is designed to work with Third-Party Services that **you** select and connect, including:
  ...
  (a) **AI model providers** — cloud providers (for example, OpenAI, Anthropic (Claude) or Google Gemini, accessed through an API key or subscription you hold, including subscription-based command-line channels) or local model runtimes exposing an OpenAI-compatible endpoint (for example, Ollama or similar) running on your own device. You may connect one or more providers; where available in your version, you can choose a provider and capability tier for each chat;

  (b) **remote access** — currently Tailscale Funnel, using your own Tailscale account (any free tier, plans and pricing are the provider's own); and

  (c) **identity for publishing** — a sign-in provider (Google or GitHub, at your choice) used only if you choose to publish to the Community Library.
...
6.4 **Distribution infrastructure.** We use third-party infrastructure — currently GitHub, Inc. for the public content warehouse (the `vaenyxai/community` repository) and the Software's source repository, and Cloudflare, Inc. for serving the Community Library index and hosting the publish service — to deliver the Software and the Community Library. The Software reads the published Community Library index from Cloudflare only; it does not contact GitHub to use the Community Library, and you do not need an account with either provider to use it. We may change infrastructure providers without notice.
...
7.2 **What we receive, honestly stated; we never train on Your Data.** We do not use Your Data to train AI models, and we have no remote access to your Instance. Data leaves your Instance, and can reach us, only through six channels: (a) content you send to a Third-Party Service you connected (clause 6.3); (b) content you choose to publish — publishing also transmits your verified sign-in identity to us (clause 8.6); (c) de-identified improvement patterns derived from your use of the Software, which — **only if you have affirmatively turned sharing on through the choice described in clause 7.4** — are shared with us and published to the Community Library (clause 7.4); (d) Your Data transmitted through remote access you enable (clause 5.3); (e) backup files you direct to a destination of your choosing, including any cloud service you point at them (clause 5.4); and (f) routine service metadata — such as your IP address, request timing and logs, and Software version strings — received by us and our infrastructure providers whenever the Software contacts our distribution infrastructure or the Operator Services (for example, to refresh the Community Library index (clause 6.4), check for or download updates (clause 4.7), publish, share improvement patterns, or submit an optional thanks or rating for a community item — a submission that also carries the item identifier and the thanks or rating itself, needs no account, and is not linked to an account unless you are signed in when you submit it), with handling and retention described in the Privacy Policy. Of these, only channels (b) and (c) send the content of Your Data to us, and channel (f) sends us operational metadata (and any thanks or rating you choose to submit) rather than the content of Your Data; channels (a), (d) and (e) send data to parties and destinations you choose, not to us. The Privacy Policy describes these channels in more detail (see clause 7.6 for its status).
...
7.4 **Community sharing (the "flywheel").** At installation you are presented — alongside, but as a step distinct from, your acceptance of these Terms (clause 1.5) — with a prominent choice about whether de-identified improvement patterns from your use may be shared with the community. **You must make this choice affirmatively: no option is pre-selected, and no sharing occurs unless and until you choose to turn sharing on.** The option to keep sharing on may be recommended and visually highlighted, but it is never pre-ticked, and declining is equally available in the same step. Your choice can be changed at any time in Settings (automatic / review-each / off). Where you have turned sharing on and sharing occurs:

  (a) sharing is designed so that only de-identified improvement patterns — not your raw data — leave your device: de-identification is applied on your device to every example before it is sent. De-identification and classification are automated processes and, like all automated processes, are imperfect: they reduce risk but cannot guarantee that no personal information will ever pass through. If the de-identification gate fails and personal information is shared, we will remove that material from the operator-controlled warehouse as described in clause 8.5 and will notify known mirrors and request its removal; the licences granted under this clause never extended to that personal information (see the de-identification-failure paragraph of this clause 7.4); Privacy Policy clause 8.6 describes the full handling protocol and cannot reduce these commitments;
...
7.5 **Publishing account data.** You do not need any account to use the Software or the Community Library. If you choose to publish, we store the minimum account information needed for attribution, management of your published content, and Merit; your public byline is a display name you choose and can edit, and your verified sign-in identity is not published (see the Privacy Policy and Contributor Agreement).

7.6 **Privacy Policy.** Our handling of any personal information we actually receive is described in the Vaenyx Privacy Policy. The Privacy Policy is a notice describing our information-handling practices, not a contract term: at installation you are asked to acknowledge it, not to agree to it, and it is not incorporated into these Terms. If these Terms and the Privacy Policy differ in their description of a privacy matter, the Privacy Policy's description prevails only to the extent it describes our practices more protectively of you: the Privacy Policy can never reduce a protection stated in these Terms, narrow the Sensitive Categories, weaken the confirmation requirement in clause 7.4(b), or widen any content channel described in clause 7.2(a)–(e) — any such change can be made only by amending these Terms under clause 18.2. The Privacy Policy may, and must, fully describe our actual handling of the operational service metadata in clause 7.2(f); doing so accurately does not widen a data channel.
...
9.1 **Nature.** Merit is a record of creator reputation reflecting the usefulness of contributions. It exists solely as recognition.
...
15.1 **Exclusion of certain losses.** Subject to clause 14, and to the maximum extent permitted by law, neither party is liable to the other for any indirect, incidental, special, or consequential loss, or for loss of profits, revenue, goodwill, or business opportunity, or for loss or corruption of data, however arising (including in contract, tort including negligence, statute or otherwise), even if advised of the possibility of that loss.

15.2 **Cap.** Subject to clauses 14 and 15.3, and to the maximum extent permitted by law, our total aggregate liability to you for all claims arising out of or in connection with these Terms, the Software, the Operator Services, Community Content or Output is limited to AUD $100 (reflecting that the Software is supplied free of charge).

15.3 **What is not limited.** Nothing in this clause 15 excludes or limits: (a) liability under a non-excludable consumer guarantee (which is instead addressed by clause 14); (b) liability that cannot be excluded or limited under applicable law, including liability for fraud or for death or personal injury caused by negligence to the extent the law does not permit its exclusion; or (c) our liability arising from: our negligent handling of the publisher account information we hold (clauses 7.5 and 8.6); our negligent operation of the improvement-sharing endpoint or of the publish service's de-identification enforcement (clauses 7.4 and 8.6(b)); our breach of the removal commitments in clauses 7.4(a) and 8.5; or our unlawful acts or omissions in operating the publish service and publisher accounts — the cap in clause 15.2 does not apply to the liability described in this clause 15.3.
...
18.1 **Changes to the Software and Operator Services.** We may modify, suspend, or discontinue any part of the Software or the Operator Services at any time. Because your Instance is self-hosted, discontinuing an Operator Service does not remove the Software from your device, although features that depend on that service may stop working.

18.2 **Changes to these Terms.** We may update these Terms from time to time. We will make the updated Terms available in the product and/or the source repository with an updated effective date, and for material changes we will give reasonable advance notice through the product or repository. Changes do not apply retroactively. Your use of the Operator Services after the updated effective date, or your installation of an update released under the updated Terms, constitutes acceptance of the updated Terms. If you do not accept the updated Terms, you may continue to use the version of the Software you have installed under the Terms you last accepted (or under any open-source licence applicable to code you hold), but you must not use the Operator Services governed by the updated Terms or install updates released under them.
...
19.3 **Entire agreement.** These Terms, together with the documents incorporated by reference (the Contributor Agreement, for publishers; the Trademark Policy; the THIRD_PARTY_NOTICES; and any applicable open-source licence), are the entire agreement between you and us about their subject matter and supersede prior understandings. The Privacy Policy is a notice describing our information-handling practices, not an incorporated contract term (clause 7.6). The CC BY 4.0 public licence operates in addition, on its own terms, for published content and shared examples (clauses 7.4(d) and 8.6(e)).

19.4 **Assignment.** You may not assign these Terms without our written consent (which will not be unreasonably withheld). We may assign these Terms to a successor of the Vaenyx project (for example, on a restructure or transfer of the project, including the incorporation of the operating company) provided the successor assumes our obligations; we will make any such assignment known through the product or repository.
