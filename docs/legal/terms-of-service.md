...
7.4 **Community sharing (the "flywheel").** Sharing is not available in this release (clause 3.0(e)). A sharing preference recorded now is a local preference: it authorises no upload and grants no licence and no moral-rights consent.
  ...
  No licence granted under these Terms ever extends to third-party personal information, or to any other material you did not hold the rights to license. If such material becomes public through a failure of a de-identification control we operate, no licence attaches to it, we will remove it under clause 8.5 and take reasonable steps to have it removed from mirrors we know of.
...
8.3 **Safety measures are risk-reduction, not guarantees.** The design's principal safety property is structural: Community Content is declarative only (clause 8.1), so an installed item is a recipe the Software interprets, not foreign code that runs. Beyond that, the safety measures actually operating in your version — such as content-hash binding, name checks at submission, or automated format and static checks — are identified in the Schedule. We describe no measure here that is not implemented. Any such measure **reduces risk but does not guarantee** that any Community Content is safe, accurate, lawful or fit for your purpose, and we do not warrant that they will detect every problem.
...
8.5 **Moderation and takedown.** Publishing to the Community Library is not pre-moderated: published items go live after automated checks, without prior human review (clause 8.6). We operate a reactive moderation process through an operator-controlled back office: we may label, re-classify, suspend, remove or take down Community Content, and may revoke Verified status or creator privileges, at any time, including in response to reports; and we may bar a publisher from further publishing (including, where appropriate, taking down that publisher's published items) in accordance with clause 17.2, including without prior notice where clause 17.2(b) applies. This process involves no access by us to your Instance (clause 3.2). Anyone may report content, whether or not they use Vaenyx and without holding any account: contact hello@vaenyx.ai, an address we publish so that it can be found and used by someone who has never installed the Software. email to hello@vaenyx.ai is handled on overseas servers, so include only what is needed for us to act, and avoid attaching personal information where an item link or identifier is enough. A report should identify the content (URL or item identifier), explain what is said to be wrong with it (for a defamation complaint, the imputations complained of), and give the complainant's name and contact details — a report containing these particulars is a duly-made complaint. For a complaint that content is defamatory, a complaint is duly made as soon as we receive, through an accessible complaints mechanism, a written complaint that (a) identifies the plaintiff (the person of or concerning whom the matter is alleged to have been published); (b) identifies the matter complained of and its location well enough for us to locate it (for example, a URL or item identifier); and (c) states that the plaintiff considers the matter to be defamatory of them. This threshold reflects the statutory minimum for a concerns notice under section 31A of the *Defamation Act 2005* (NSW) (and its interstate equivalents); the poster's identity, the specific imputations complained of, the reasons the matter is claimed to be defamatory, and the complainant's further contact details and any supporting evidence are helpful particulars that we may request, but their absence does not stop a complaint meeting (a)–(c) from being duly made. A report that lacks some of these particulars is still received, triaged and assessed; the particulars affect only whether it is a duly-made complaint for the purposes of this clause. We act on reports as soon as reasonably practicable, and we treat material capable of causing serious harm as our first priority ahead of every other report. Where a law or a regulator's notice sets a period for removing or disabling access to particular content, we comply within that period. We do not publish a fixed response time of our own: this is a small operation, and a stated clock we cannot always meet would be a promise, not a service. For a duly-made complaint that Community Content is unlawful, including that it is defamatory, we will assess the complaint and take the steps (if any) to remove or disable access to the content that are reasonable in the circumstances. We assess sufficiently detailed reports having regard to apparent urgency, potential harm, the information available to us and applicable legal requirements. We may restrict content or accounts without prior notice. Submitting a report does not guarantee removal, restoration, reasons, individual correspondence or any particular outcome. We may notify a publisher whose content we restrict, may consider anything they send in reply, and may restore content — but none of those is an obligation, and we do not decide whether reported material is in fact unlawful. Where published or shared material is found to contain personal information in breach of the de-identification promise (clause 7.4(a)), we may take reasonable steps within our control to restrict access to it, which may include removing it from the catalogue and index installations read, removing the published files, blocking republication of that identifier, and where we consider it appropriate rewriting repository history. We do not control forks, mirrors, caches or archives held by others, and make no commitment about copies outside our control. Removal from the Community Library does not automatically delete copies already installed on users' devices, although your Instance — acting on the published library index it reads — may locally surface warnings about, or disable, affected items where feasible; this mechanism involves no access by us to your Instance.

8.6 **Publishing.** Everything you create defaults to your private local library (shown in the product as the "Library"). Methods and Routines you create reach the public Community Library only if you explicitly publish them. Community sharing of improvement examples is not available in this release (clauses 3.0(e) and 7.4). Publishing:

  (a) **requires sign-in** through one of the sign-in providers offered for your version, identified in the Schedule and at the sign-in step. No option confers an advantage over another, and none requires an account with the host of the public repository. Your public byline is a self-chosen, editable display name; your verified sign-in identity remains private;

  (b) **occurs through our publish service.** A publication submission contains only the declarative item files, your account identifier, the public byline you choose and the acceptance records identified in the Schedule. It does not include Examples or raw corrections, no Example de-identification or preview is performed on it, and no Merit is recorded for it. Local examples and raw corrections are not sent or published. If a later version permits examples to be attached, that feature will not be enabled until the Software de-identifies them on-device, shows you the exact material proposed for publication, and requires your express approval. Your Instance sends those files to the publish service we operate; that service alone holds the credentials used to write to the public content warehouse — the Software never holds or stores any warehouse write token on your machine. The service checks the **shape** of the submission — that the files are of the permitted declarative kinds, that their paths are safe, and that they are within size limits — and rejects a non-conforming submission rather than rewriting it. It also enforces that an item identifier stays with the account that first published it, and that a removed item cannot be republished. **It does not check what the words say.** A recipe is written in ordinary language, so the automated checks cannot tell you that its contents are accurate, lawful or safe; that is what clause 8.2, clause 8.5 and the Contributor Agreement are for. The service then stamps your display name into the published item and commits it to the public repository;
...
15.3 **What is not limited.** Nothing in this clause 15 excludes or limits:
  ...
  (d) either party's liability for knowing infringement of a third party's intellectual-property rights, for knowing unlawful disclosure of another person's personal information, or for a serious invasion of privacy committed intentionally or recklessly. In this clause **"knowing" means actual knowledge of the relevant facts, or deliberately shutting one's eyes to them**; receiving a report or allegation is not by itself knowledge of the matter alleged;
  ...
  (f) our liability for a **deliberate or reckless** breach of an express privacy, confidentiality or security obligation under these Terms or applicable law, for deliberately or recklessly mishandling personal or confidential information we hold or control, or for deliberately or recklessly failing to honour an express removal commitment. Liability for an ordinary, non-reckless mistake in operating those processes remains subject to the caps in clause 15.2. This paragraph is not a licence to be careless: negligence still gives rise to liability, but it is liability within the cap rather than outside it.
