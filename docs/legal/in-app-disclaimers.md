...
- **Key change at 2.6:** this card moves from `legal.consent.flywheel.*` to `legal.notice.flywheel.preference.*`. Retire `legal.consent.flywheel.title`, `.body`, `.accept`, `.decline`, `.sensitiveNote` and `.multiUserNote`: they describe an always-ask gate and a multi-user sharing rule for a capability that does not exist, and `.accept` currently reads "Keep Sharing On (Recommended)" — a recommendation to consent to nothing. New keys: `legal.notice.flywheel.preference.title`, `.body`, `.interested`, `.notInterested`.
