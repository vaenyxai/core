...
- **⚠ 上线阻断项:** 在邮箱迁移到具备经核实数据处理条款、并有已实现留存流程的服务商之前,本数据流不得在生效日期时仍处于启用状态。迁移后,本条目须载明实际服务商、接收方、国家、保障措施与经核实的删除规则。仅作出海外处理的披露,不视为已符合 APP 8.1。
...
- **问题:** hello@vaenyx.ai 的来信落入不受经核实数据处理条款约束的邮箱。就该数据流,不存在已核实的 APP 8.1 合同保障。
- **备注:** 与上方 Cloudflare 一项相对照:就该邮箱,不存在同等的已核实立场。
- **所需行动:** 把邮箱迁移到受数据处理条款约束的服务商,或就该数据流重新确立合法依据。仅作披露不等于合规。
