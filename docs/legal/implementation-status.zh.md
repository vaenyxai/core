...
### 订阅登录型模型渠道 — `external-unverified`

- **问题:** 默认的聊天后端不是用 API key,而是经 Codex CLI 用用户自己的 ChatGPT 订阅登录。消费级订阅能否经由第三方产品使用,决定权在提供方,而两家之中只有一家给了答案。Anthropic 在其官方帮助中心给了,答案是可以:第三方应用可以经 Agent SDK 用用户的 Claude 订阅认证。但这类用量算在哪里,四个月内变了三次 —— 2026 年 4 月全面禁止,其后改为另设的、按量计费的 Agent SDK 月度额度,再到 2026 年 6 月 15 日该变更被暂停,因此目前算在订阅本身的常规用量额度内。已获许可、已公开、且不稳定。OpenAI 则未作任何表态。有人直接询问「经修改的 Codex CLI 或另行分发的应用是否可以这样认证」,维护者的回答是条款相当宽松、建议咨询法律专家。那不是许可;而且 ChatGPT 条款另有限制自动化或程序化使用的条文。
- **备注:** 本条针对的是运营方自身的行为,不是用户的行为。条款已把每一项第三方服务放在用户手里 —— 他的选择、他的合同、他的费用 —— 用户自己粘贴 API key 的情形完全适用。但在由运营方编写集成、去驱动一个消费级订阅的情形下,它的作用要小得多:提供方在那里禁止的是产品,而用户接受我们的条款,并不能为我们的代码取得许可。因此如实记为未核实事项,而不是单靠合同上的责任分配了事。
- **所需行动:** 就「分发给他人使用的应用,能否让每位用户各自用自己的订阅经 Sign in with ChatGPT 认证」向 OpenAI 取得书面答复。在取得之前,本条就该渠道持续有效,任何材料均不得把它描述为经提供方批准、认可或涵盖。Anthropic 那条渠道无需此答复 —— 它已公开许可 —— 但由于额度归属四个月内变了三次,任何材料均不得声称订阅「包含」多少;只描述「使用你自己的方案、受其额度限制」。
