...
> 明细表版本 **2026-07-29.1** · 自以下日期起生效 **2026-07-26** · 核实于 **2026-07-29**
...
- **问题:** 默认的聊天后端不是用 API key,而是经 Codex CLI 用用户自己的 ChatGPT 订阅登录。消费级订阅能否经由第三方产品使用,决定权在提供方,而两家提供方的答案并不相同。Anthropic 已公开表态:2026 年 4 月禁止在第三方工具中使用订阅凭据,随后又以另设的、按量计费的 Agent SDK 额度形式重新开放 —— 这是一条明确的路径,但三个月内已改动两次。OpenAI 则未作任何表态。有人直接询问「经修改的 Codex CLI 或另行分发的应用是否可以这样认证」,维护者的回答是条款相当宽松、建议咨询法律专家。那不是许可;而且 ChatGPT 条款另有限制自动化或程序化使用的条文。
...
- **所需行动:** 就「分发给他人使用的应用,能否让每位用户各自用自己的订阅经 Sign in with ChatGPT 认证」向 OpenAI 取得书面答复。在取得之前,本条持续有效,任何材料均不得把该渠道描述为经提供方批准、认可或涵盖。若日后确实要建通往 Anthropic 的渠道,应走已公开的 Agent SDK 额度路径而非订阅令牌,并如实描述为按月计量的额度,而非「订阅内包含」。
